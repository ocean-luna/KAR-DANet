...
class ResNet(nn.Module):
    def __init__(self,
                 ...
                 groups=1,
                 width_per_group=64,
                 self.kan_output_dim = num_classes):  # 这里假设 KAN 层的输出维度为类别数量
        """
    ...
    #         # x = self.linear(x)
    #         x = self.kan(x)
    #         print("Output tensor shape:", x.shape)  # 打印输出张量的形状
    #     #assert x.dim() == 2 and x.size(1) == self.in_features
    #     assert x.dim() == 2 and x.size(1) == self.kan_output_dim
    ...
    # def grad_cam(self, feature_map, target_layer):
    #    """
    #     生成 Grad-CAM 热力图

    #      参数:
    #        feature_map (torch.Tensor): 经过 KAN 层后的特征图。
    #         target_layer (torch.nn.Module): 目标层，用于计算梯度。

    #        返回:
    #        torch.Tensor: Grad-CAM 热力图。
    #    """
    #    # 首先将模型设为评估模式，以便获取梯度
    #    self.eval()
    ...
    #    output = self.get_kan_output(feature_map)
    ...
    #    # 将热力图归一化为 [0, 1]
    #    heatmap = torch.relu(heatmap)  # 修剪负值
...
for epoch in range(50):
    ...
    with tqdm(trainloader, total=len(trainloader)) as pbar:
        # 遍历训练数据加载器中的每个批次
    ...
    # 禁用梯度计算
    #with torch.no_grad():
    with torch.no_grad(), tqdm(valloader, total=len(valloader)) as pbar:
        # 遍历验证数据加载器中的每个批次
...
# ################加入Grad-CAM可视化#########################
# # 在训练和验证过程中应用Grad-CAM可视化
# # 训练和验证循环中的 Grad-CAM 可视化
...
#         if i == 0:  # 只在每个 epoch 的第一个批次中执行 Grad-CAM
#             # 获取 KAN 层的输出
#             kan_output = model.get_kan_output(images)
#             # 应用 Grad-CAM 可视化
#             heatmap = model.grad_cam(kan_output, target_layer=model.kan)
#             heatmap_np = heatmap.detach().cpu().numpy()
#             heatmap_np = cv2.resize(heatmap_np, (kan_output.shape[2], kan_output.shape[3]))
#             heatmap_np = np.uint8(255 * heatmap_np)
#             heatmap_np = cv2.applyColorMap(heatmap_np, cv2.COLORMAP_JET)
#             superimposed_img = heatmap_np * 0.4 + images[0].permute(1, 2, 0).cpu().numpy() * 255
#             superimposed_img = superimposed_img / np.max(superimposed_img)
#             cv2.imwrite(f'gradcam_epoch{epoch}.jpg', superimposed_img)
...
plt.legend()
plt.savefig('kan_resnet_seconddata_2.png')  # 保存绘制的图像
plt.show()